...
\end{knitrout}
			...
			Para el cálculo del umbral de confianza, se irán tomando conjuntos $\beta$ y $\alpha$ de forma que se vayan evaluando asociaciones de la forma $\alpha \rightarrow \beta - \alpha$ y se pueda aplicar el \Cref{th:ap}. Se selecciona el $\beta$ de mayor dimensión, y para cada $\beta, \alpha \subset \beta$. 
			
			\begin{observation}
				Como el \Cref{th:ap} es de la forma $\lnot p \rightarrow \lnot q$, siempre que se tiene $\lnot p$ se deduce $\lnot q$ por la regla de resolución, pero si se tiene $p$, no siempre se deduce $\lnot q$, por lo que hay que analizar $\forall i \, \alpha'_i \rightarrow \beta - \alpha'_i$.
				\label{obs:ap}
			\end{observation}
