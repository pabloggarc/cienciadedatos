...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{factorial} \hlkwb{=} \hlkwa{function}\hlstd{(}\hlkwc{n}\hlstd{) \{}
\hlkwa{if} \hlstd{(n} \hlopt{<=} \hlnum{1}\hlstd{)} \hlkwd{return}\hlstd{(}\hlnum{1}\hlstd{)}
\hlstd{n} \hlopt{*} \hlkwd{factorial}\hlstd{(n}\hlopt{-}\hlnum{1}\hlstd{)}
\hlstd{\}}
...
\end{knitrout}
			...
			Las dos siguientes son \texttt{cabeza} y \texttt{cola}. Dada una lista y un valor natural, \texttt{cabeza} devuelve otra lista que es una partición de la original hasta el índice del valor indicado. De la misma forma, \texttt{cola} devuelve la lista que contiene los últimos valores indicados. 
			
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{cabeza} \hlkwb{=} \hlkwa{function}\hlstd{(}\hlkwc{l}\hlstd{,} \hlkwc{n}\hlstd{) \{}
        \hlkwa{if} \hlstd{(n} \hlopt{<=} \hlnum{0}\hlstd{)} \hlkwd{return}\hlstd{(}\hlkwa{NULL}\hlstd{)}
        \hlkwd{head}\hlstd{(l, n)}
\hlstd{\}}

\hlstd{cola} \hlkwb{=} \hlkwa{function}\hlstd{(}\hlkwc{l}\hlstd{,} \hlkwc{n}\hlstd{) \{}
        \hlkwa{if} \hlstd{(n} \hlopt{<=} \hlnum{0}\hlstd{)} \hlkwd{return}\hlstd{(}\hlkwa{NULL}\hlstd{)}
...
\hlstd{binom} \hlkwb{=} \hlkwa{function}\hlstd{(}\hlkwc{n}\hlstd{,} \hlkwc{k}\hlstd{) \{}
        ...
        \hlkwa{for} \hlstd{(i} \hlkwa{in} \hlnum{0}\hlopt{:}\hlstd{n) \{}
                \hlkwa{for} \hlstd{(j} \hlkwa{in} \hlnum{0}\hlopt{:}\hlkwd{min}\hlstd{(i, k)) \{}
                        ...
                        \hlstd{\}}
                        \hlkwa{else} \hlstd{\{}
                                \hlstd{temp[i} \hlopt{+} \hlnum{1}\hlstd{, j} \hlopt{+} \hlnum{1}\hlstd{]} \hlkwb{=} \hlstd{temp[i, j]} \hlopt{+} \hlstd{temp[i, j} \hlopt{+} \hlnum{1}\hlstd{]}
                        \hlstd{\}}
                \hlstd{\}}
        \hlstd{\}}
...
\end{knitrout}
			...
			Finalmente, la función \texttt{fk\_1} representa el algoritmo al completo. El algoritmo funciona alrededor de ir añadiendo subconjuntos candidatos calculados en iteraciones previas a un vector. El algoritmo recibe una lista de los sucesos elementales que han superado el umbral de soporte establecido en el paso previo del algoritmo, y con \texttt{lapply} añade al vector mencionado previamente los que serían los conjuntos candidatos de dimensión 1. El algoritmo sigue con las sucesivas dimensiones, y por ejemplo, en la dimensión $k$ combina los sucesos candidatos de dimensión $k-1$ dos a dos, obtenidos en la iteración previa. Para cada posible pareja, se comprueban las dos reglas explicadas previamente con ayuda de las funciones \texttt{cabeza} y \texttt{cola}. Aquellas que verifican ambas reglas, se añaden al vector comentado como candidatos de dimensión $k$. \\
...
\hlstd{fk_1} \hlkwb{=} \hlkwa{function}\hlstd{(}\hlkwc{clasif}\hlstd{) \{}
        ...
        \hlstd{inicio} \hlkwb{=} \hlnum{1}

        \hlkwa{for} \hlstd{(k} \hlkwa{in} \hlnum{2}\hlopt{:}\hlstd{n) \{}
                \hlkwa{for} \hlstd{(i} \hlkwa{in} \hlstd{inicio}\hlopt{:}\hlstd{(n_comb}\hlopt{-}\hlnum{1}\hlstd{)) \{}
                        \hlkwa{for} \hlstd{(j} \hlkwa{in} \hlstd{(i}\hlopt{+}\hlnum{1}\hlstd{)}\hlopt{:}\hlstd{n_comb) \{}

                                \hlstd{cab1} \hlkwb{=} \hlkwd{cabeza}\hlstd{(comb[[i]],} \hlkwd{len}\hlstd{(comb[[i]])}\hlopt{-}\hlnum{1}\hlstd{)}
                                \hlstd{cab2} \hlkwb{=} \hlkwd{cabeza}\hlstd{(comb[[j]],} \hlkwd{len}\hlstd{(comb[[j]])}\hlopt{-}\hlnum{1}\hlstd{)}

                                \hlstd{col1} \hlkwb{=} \hlkwd{cola}\hlstd{(comb[[i]],} \hlnum{1}\hlstd{)}
                                \hlstd{col2} \hlkwb{=} \hlkwd{cola}\hlstd{(comb[[j]],} \hlnum{1}\hlstd{)}

                                \hlkwa{if} \hlstd{(}\hlkwd{equals}\hlstd{(cab1, cab2)} \hlopt{& !}\hlkwd{equals}\hlstd{(col1, col2)) \{}
                                        \hlstd{new} \hlkwb{=} \hlkwd{c}\hlstd{(cab1, col1, col2)}
                                        \hlstd{comb} \hlkwb{=} \hlkwd{c}\hlstd{(comb,} \hlkwd{list}\hlstd{(new))}
                                \hlstd{\}}
                        \hlstd{\}}
                \hlstd{\}}
                \hlstd{inicio} \hlkwb{=} \hlstd{n_comb} \hlopt{+} \hlnum{1}
                \hlstd{n_comb} \hlkwb{=} \hlstd{n_comb} \hlopt{+} \hlkwd{factorial}\hlstd{(n)} \hlopt{/} \hlstd{(}\hlkwd{factorial}\hlstd{(k)} \hlopt{*} \hlkwd{factorial}\hlstd{(n}\hlopt{-}\hlstd{k))}
        \hlstd{\}}
        \hlkwd{cola}\hlstd{(comb, n_comb}\hlopt{-}\hlstd{n)}
\hlstd{\}}
\end{alltt}
\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in create\_comb(table, soporte\_clasif, s): no se pudo encontrar la función "{}create\_comb"{}}}\end{kframe}
\end{knitrout}
...
\hlstd{get_n_set} \hlkwb{=} \hlkwa{function}\hlstd{(}\hlkwc{df}\hlstd{,} \hlkwc{k}\hlstd{) \{}
        ...
        \hlkwa{for} \hlstd{(column} \hlkwa{in} \hlnum{1}\hlopt{:}\hlkwd{len}\hlstd{(distance_matrix[}\hlnum{1}\hlstd{,]))\{}
                ...
                \hlstd{ordered_column[k_aux,} \hlstr{"dis"}\hlstd{]} \hlopt{==} \hlstd{ordered_column[k_aux}\hlopt{+}\hlnum{1}\hlstd{,} \hlstr{"dis"}\hlstd{]) \{}
...
\hlstd{get_densities} \hlkwb{=} \hlkwa{function}\hlstd{(}\hlkwc{n_set}\hlstd{) \{}
        ...
        \hlkwa{for} \hlstd{(i} \hlkwa{in} \hlnum{1}\hlopt{:}\hlkwd{len}\hlstd{(n_set[,}\hlnum{1}\hlstd{])) \{}
                \hlstd{densities} \hlkwb{=} \hlkwd{append}\hlstd{(densities,} \hlkwd{len}\hlstd{(n_set[i,}\hlnum{1}\hlstd{][[}\hlnum{1}\hlstd{]])} \hlopt{/}
                 \hlkwd{add_list}\hlstd{(n_set[i,}\hlnum{1}\hlstd{][[}\hlnum{1}\hlstd{]]))}
        \hlstd{\}}
        \hlstd{densities}
\hlstd{\}}
\end{alltt}
