...
\newtcbtheorem[number within = section, Crefname = {Teorema}{Teorema}, fonttitle = \bfseries]{teorema}{Teorema}{}{th}
\newtcbtheorem[number within = section, Crefname = {Definición}{Definición}, fonttitle = \bfseries]{definicion}{Definición}{}{df}
...
\begin{document}
	...
	\chapter*{Introducción}\addcontentsline{toc}{chapter}{Introducción}\pagestyle{plain}
		...
		\section*{El lenguaje \LaTeX}\addcontentsline{toc}{section}{El lenguaje \LaTeX}
			...
			Para ello será necesario fijarse en la \Cref{fig:extensiones}. Por ahora se utilizarán las extensiones con las que se trabajará más a menudo, que serán \texttt{.Rnw}, \texttt{.tex}, y \texttt{.pdf}. La primera de ellas representa los archivos que tienen código \LaTeX{} y R ``mezclado'', la segunda aquellos que contienen código \LaTeX{} puro, y la última el documento final. \\
...
\end{knitrout}
			...
			Además, es posible introducir nuevos criterios a la ordenación, como por ejemplo, hacerlo en orden descendente. Para esto se utilizará la función \texttt{rev}.
...
\end{knitrout}
			...
			También suele ser útil conocer cuántos elementos tiene una columna. Es posible averiguarlo con la función \texttt{length}, véase un ejemplo.
...
\end{knitrout}
			...
			Otro valor que es posible calcular es el rango. Para ello se utilizan las funciones \texttt{max} y \texttt{min}. Es necesario tener cuidado con la función \texttt{range}, pues esta devuelve los valores máximo y mínimo de la columna. 
...
\end{knitrout}
			...
			Para una mejor lectura, es posible cambiar la forma de obtener la columna de los radios: 
...
\end{knitrout}
			...
			Sin embargo, al salir de R, la definición de la función se pierde, por lo que para mantenerla es necesario guardarla en un fichero, y posteriormente cargarlo en futuras ejecuciones. Esta acción se realiza de la siguiente manera:
...
\end{knitrout}
			...
			Retomando el estudio de los datos, va a explicarse cómo calcular las diferentes frecuencias. Como en R no existe una función para las frecuencias relativas, se definirá y guardará una propia. 
...
\end{knitrout}
					...
					Como es posible visualizar en \texttt{\$out}, se obtiene que el suceso outlier es el 14, que como se vió en el ejercicio de la clase de teoría es correcto. Sin embargo, si al fijarse en \texttt{\$conf}, se pueden ver los límites del intervalo, y estos no coinciden con los vistos en clase. Esto es debido a que, al igual que ocurría al usar la función \texttt{sd} para el cálculo de la desviación, R utiliza un factor de $n-1$ en vez de $n$, por lo que no son los valores exactos. \\
					
					Tras codificar el algoritmo caja y bigotes, se va a realizar la identificación de outliers mediante el cálculo de los cuartiles. Además, se han calculado los límites del intervalo mediante la siguiente ecuación.
...
\end{knitrout}
				...
				\subsubsection{Media y desviación}
					...
					Otra forma de buscar sucesos outliers utilizando técnicas con base estadística es mediante el uso de la media y la desviación típica. Para mostrar esta técnica, se va a utilizar la otra columna del dataframe (\texttt{densidad}). A través el uso de las funciones \texttt{mean} y \texttt{sd} se obtienen ambos valores, que son unidos en los límites del intervalo mediante la siguiente ecuación.
...
\end{knitrout}
					...
					Se observa que los valores inferior y superior del intervalo no son iguales que en el ejercicio realizado en clase, esto se debe a que, como ya se vio, la desviación típica se calcula con un factor de $n-1$ en vez de $n$, por lo que arreglando el cálculo de la desviación quedaría así.
...
\end{knitrout}
			...
			Para esta función se han utilizado las funciones \texttt{append} (paquete \texttt{base}), que devuelve un vector con la lista que se pasa como parámetro y el elemento que se introduce como parámetro concatenado a la derecha del mismo, y la función \texttt{rbind} (paquete \texttt{base}), que devuelva una matriz con los vectores que se pasan como parámetros a modo de fila. Se comprueban las frecuencias absolutas:
...
\end{knitrout}
			...
			Se puede comprobar que los resultados son correctos, ya que la frecuencia absoluta acumulada del último elemento es igual al número total de elementos (73) y la frecuencia relativa acumulada del último elemento es 1.\\
			
			Prosiguiendo con el análisis de datos, se debe que obtener la media aritmética del conjunto de datos. Para ello, se propone la siguiente función, la cual va a sumar todos los elementos de la muestra, y ese resultado se divide entre el número total de elementos de la muestra, en definitiva: 
			
			$$
			\bar{x}_a = \frac{\displaystyle\sum_{i=1}^n x_i}{n}
			$$
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{(media_aritm} \hlkwb{=} \hlkwd{fcd_mean}\hlstd{(distancias))}
\end{alltt}
\begin{verbatim}
## [1] 18.53425
\end{verbatim}
...
\end{knitrout}
			...
			En resumen, el cálculo obedece a la siguiente fórmula: 
			
			$$
			\forall i \in \{c, d, p\}\,\, \tilde{x}_i = \left\lbrace \begin{array}{cll}
				x_{\left\lceil ni \right\rceil} & \text{si} & ni \not\in\mathbb{N}\\
				&&\\
				\dfrac{x_{ni}+x_{ni+1}}{2} & \text{si} & ni \in\mathbb{N}
			\end{array}\right.
			$$
...
\hlstd{union} \hlkwb{=} \hlkwa{function}\hlstd{(}\hlkwc{c1}\hlstd{,} \hlkwc{c2}\hlstd{) \{}
        \hlkwa{if} \hlstd{(}\hlkwd{len}\hlstd{(c1)} \hlopt{==} \hlnum{0}\hlstd{) \{}
                \hlstd{c2}
        \hlstd{\}} \hlkwa{else if} \hlstd{(c1[}\hlnum{1}\hlstd{]} \hlopt{%in%} \hlstd{c2) \{}
...
\end{knitrout}
			...
			Para ambas funciones se ha hecho uso del operador \texttt{\%in\%}, el cual determina si un elemento se encuentra en una lista. \\
...
\end{knitrout}
			...
			De la misma manera que se ha necesitado calcular el soporte, también se necesitará calcular la confianza, definida como se muestra en la \Cref{df:cf}. Se implementa con la función \texttt{confidence}. 
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{ap_genrules} \hlkwb{=} \hlkwa{function}\hlstd{(}\hlkwc{table}\hlstd{,} \hlkwc{assoc}\hlstd{,} \hlkwc{c}\hlstd{) \{}
        \hlstd{assoc} \hlkwb{=} \hlkwd{cbind}\hlstd{(assoc,} \hlkwd{data.frame}\hlstd{(}\hlkwd{matrix}\hlstd{(}\hlnum{1}\hlstd{,} \hlkwc{ncol} \hlstd{=} \hlnum{1}\hlstd{,}
                \hlkwc{nrow} \hlstd{=} \hlkwd{len}\hlstd{(assoc[,}\hlnum{1}\hlstd{]),} \hlkwc{dimnames} \hlstd{=} \hlkwd{list}\hlstd{(}\hlnum{1}\hlopt{:}\hlkwd{len}\hlstd{(assoc[,}\hlnum{1}\hlstd{]),} \hlstr{"valid"}\hlstd{))))}
        \hlkwa{for} \hlstd{(i} \hlkwa{in} \hlkwd{len}\hlstd{(assoc[,}\hlstr{"left"}\hlstd{])}\hlopt{:}\hlnum{1}\hlstd{) \{}
                \hlstd{A} \hlkwb{=} \hlstd{assoc[i,} \hlstr{"left"}\hlstd{][[}\hlnum{1}\hlstd{]]}
                \hlstd{right} \hlkwb{=} \hlstd{assoc[i,} \hlstr{"right"}\hlstd{][[}\hlnum{1}\hlstd{]]}
                \hlstd{B} \hlkwb{=} \hlkwd{union}\hlstd{(A, right)}

                \hlkwa{if}\hlstd{(assoc[i,} \hlstr{"valid"}\hlstd{]) \{}
                        ...
                        \hlkwa{if} \hlstd{(conf} \hlopt{<} \hlstd{c) \{}
                                \hlkwa{for} \hlstd{(j} \hlkwa{in} \hlnum{1}\hlopt{:}\hlkwd{len}\hlstd{(A)) \{}
                                        \hlstd{A_primes} \hlkwb{=} \hlkwd{combn}\hlstd{(A,} \hlkwc{m}\hlstd{=j,} \hlkwc{simplify}\hlstd{=}\hlnum{TRUE}\hlstd{)}
                                        \hlkwa{for} \hlstd{(k} \hlkwa{in} \hlnum{1}\hlopt{:}\hlkwd{len}\hlstd{(A_primes[}\hlnum{1}\hlstd{,])) \{}
                                                \hlstd{new_left} \hlkwb{=} \hlstd{A_primes[,k]}
                                                \hlstd{new_right} \hlkwb{=} \hlkwd{dif}\hlstd{(B, new_left)}
                                                \hlstd{index} \hlkwb{=} \hlkwd{indexOf_assoc}\hlstd{(}
                                                        \hlstd{assoc, new_left, new_right)}
                                                \hlstd{assoc[index,} \hlstr{"valid"}\hlstd{]} \hlkwb{=} \hlnum{0}
                                        \hlstd{\}}
                                \hlstd{\}}
                        \hlstd{\}}
                \hlstd{\}}
        \hlstd{\}}
        \hlkwd{subset}\hlstd{(assoc, assoc}\hlopt{$}\hlstd{valid} \hlopt{==} \hlnum{1}\hlstd{)}
\hlstd{\}}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\end{knitrout}
			...
			\begin{itemize}
				\item \textbf{Dimensión 2}\\
				\Tree[.\{1645\} [. [.\rojo{14} ]  [.\rojo{15} [.\rojo{45} ] ] [.\rojo{16} ] ] [. [.\rojo{64} ] [.\rojo{65} ] ] ]\hfill
				\Tree[.\{1356\} [. [.\rojo{15} ] [.\rojo{13} [.\rojo{16} ] ] ] [.\rojo{56} ] [. [.\rojo{35} ] [.\rojo{36} ] ] ]\hfill
				\Tree[.\{461\} [ [.\rojo{41} ] [.\rojo{46} ] ] [.\rojo{61} ] ]\\
				\Tree[.\{4315\} [. [.\rojo{41} ] [.\rojo{45} [.\rojo{15} ] ] [.\rojo{43} ] ] [. [.\rojo{31} ] [.\rojo{35} ] ] ]\hfill
				\Tree[.\{165\} [. [.\rojo{15} ] [.\rojo{16} ] ] [.\rojo{65} ] ]\hfill
				\Tree[.\{156\} [. [.\rojo{15} ] [.\rojo{16} ] ] [.\rojo{56} ] ]\hfill
				\Tree[.\{32\} [.32] ]
								
				\item \textbf{Dimensión 3}\\ 
				\Tree[.\{1645\} [. [.\rojo{145} ] [. [.\rojo{164} ] [.\rojo{165} ] ] ] [.\rojo{645} ] ]\hfill
				\Tree[.\{1356\} [. [.\rojo{156} ] [. [.\rojo{135} ] [.\rojo{136} ] ] ] [.\rojo{356} ] ]\hfill
				\Tree[.\{461\} [.\rojo{461} ] ]\\
				\Tree[.\{4315\} [. [.415 ] [. [.\rojo{431} ] [.\rojo{435} ] ] ] [.\rojo{315} ] ]\hfill
				\Tree[.\{165\} [.\rojo{165} ] ]\hfill
				\Tree[.\{156\} [.\rojo{156} ] ]
				
				\item \textbf{Dimensión 4}\\
				\Tree[.\{1645\} [.\rojo{1645} ] ]\hfill
				\Tree[.\{1356\} [.\rojo{1356} ] ]\hfill
				\Tree[.\{4315\} [.\rojo{4315} ] ]\hfill
			\end{itemize}
			
			Una vez construidos estos árboles, se hace un recuento de cuáles de sus hojas aparecen en las del grupo de árboles anterior. Los sucesos candidatos serán aquellos que aparezcan $2.4$ veces o más. \\
			
			{\centering\Tree[.{Dimensión 2} [. \boxed{14(3)} [.\boxed{15(5)} [.45(2) ] ] [.13(2) [.\boxed{16(5)} [.46(2) ] ] ] ] \boxed{56(4)} [. 34(1) 35(2) 36(1) ] ]}\\
			\Tree[.{Dimensión 3} [. [.146(2) ] [. [.154(1) ] [.\boxed{156(4)} ] ] [. [.134(1) ] [.135(2) [.465(1) ] ] [.136(1) ] ] ] [. [. [.354(1) ] [.356(1) ] ] [.364(0) ] ] ]\\
			\Tree[.{Dimensión 4} [. [.1546(1) ] [. [. [.1345(1) ] [.1346(0) ] ] [.1356(1) ] ] ] [.3546(0) ] ]\hfill
			\Tree[.{Dimensión 5} 13456(0) ]\\
			...
			Para el cálculo del umbral de confianza, se irán tomando conjuntos $\beta$ y $\alpha$ de forma que se vayan evaluando asociaciones de la forma $\beta \rightarrow \beta - \alpha$ y se pueda aplicar el \Cref{th:ap}. Se selecciona el $\beta$ de mayor dimensión, y para cada $\beta, \alpha \subset \beta$. 
			
			\begin{observation}
				Como el \Cref{th:ap} es de la forma $\lnot p \rightarrow \lnot q$, siempre que se tiene $\lnot p$ se deduce $\lnot q$, pero si se tiene $p$, no siempre se deduce $\lnot q$, por lo que hay que analizar $\forall i \, \alpha'_i \rightarrow \beta - \alpha'_i$.
				\label{obs:ap}
			\end{observation}
...
\hlstd{boxNmustaches} \hlkwb{=} \hlkwa{function}\hlstd{(}\hlkwc{sample}\hlstd{,} \hlkwc{d}\hlstd{,} \hlkwc{details} \hlstd{=} \hlnum{FALSE}\hlstd{) \{}
        ...
        \hlkwa{if}\hlstd{(details) \{}
                ...
                \hlkwd{cat}\hlstd{(}\hlstr{"Grado de outlier d = "}\hlstd{,d,}\hlstr{"\textbackslash{}n\textbackslash{}n"}\hlstd{)}

                \hlkwd{print}\hlstd{(}\hlstr{"->PASO 2: CÁLCULO CUARTILES 1 Y 3"}\hlstd{)}
                \hlkwd{cat}\hlstd{(}\hlstr{"El cuart. 1 es "}\hlstd{,cuart1,} \hlstr{" y el cuart. 3 es "}\hlstd{, cuart3,}\hlstr{"\textbackslash{}n\textbackslash{}n"}\hlstd{)}

                \hlkwd{print}\hlstd{(}\hlstr{"->PASO 3: CÁLCULO DEL INTERVALO"}\hlstd{)}
                \hlkwd{cat}\hlstd{(}\hlstr{"Fórmula -> (Q1 - d*(Q3-Q1), Q3 + d*(Q3-Q1))\textbackslash{}n"}\hlstd{)}
                \hlkwd{cat}\hlstd{(}\hlstr{"Intervalo: ("}\hlstd{,lim_inf,}\hlstr{", "}\hlstd{,lim_sup,}\hlstr{")\textbackslash{}n\textbackslash{}n"}\hlstd{)}
        ...
        \hlkwa{for} \hlstd{(i} \hlkwa{in} \hlnum{1}\hlopt{:}\hlkwd{len}\hlstd{(sample)) \{}
                \hlkwa{if}\hlstd{(sample[i]} \hlopt{<} \hlstd{lim_inf} \hlopt{||} \hlstd{sample[i]} \hlopt{>} \hlstd{lim_sup) \{}
                        ...
                        \hlkwa{if}\hlstd{(details) \{}
                                \hlkwd{cat}\hlstd{(}\hlstr{"El punto "}\hlstd{,i,}\hlstr{" con valor "}\hlstd{,sample[i],}
...
\hlstd{dispersed_outliers} \hlkwb{=} \hlkwa{function}\hlstd{(}\hlkwc{sample}\hlstd{,} \hlkwc{d}\hlstd{,} \hlkwc{details} \hlstd{=} \hlnum{FALSE}\hlstd{) \{}
        \hlstd{outliers} \hlkwb{=} \hlkwd{c}\hlstd{()}
        \hlstd{mean_sample} \hlkwb{=} \hlkwd{fcd_mean}\hlstd{(sample)}
        ...
        \hlkwa{if}\hlstd{(details) \{}
                ...
                \hlkwd{print}\hlstd{(}\hlstr{"->PASO 4: CÁLCULO DEL INTERVALO"}\hlstd{)}
                \hlkwd{cat}\hlstd{(}\hlstr{"Fórmula -> (Media - d*DesvTipica, Media + d*DesvTipica)\textbackslash{}n"}\hlstd{)}
                \hlkwd{cat}\hlstd{(}\hlstr{"El intervalo es: ("}\hlstd{,lim_inf,}\hlstr{", "}\hlstd{,lim_sup,}\hlstr{")\textbackslash{}n\textbackslash{}n"}\hlstd{)}
...
## 
## [1] "->PASO 4: CÁLCULO DEL INTERVALO"
## Fórmula -> (Media - d*DesvTipica, Media + d*DesvTipica)
## El intervalo es: ( 3.628134 ,  11.37187 )
## 
...
\hlstd{get_n_set} \hlkwb{=} \hlkwa{function}\hlstd{(}\hlkwc{df}\hlstd{,} \hlkwc{k}\hlstd{) \{}
        ...
        \hlkwa{for} \hlstd{(column} \hlkwa{in} \hlnum{1}\hlopt{:}\hlkwd{len}\hlstd{(distance_matrix[}\hlnum{1}\hlstd{,])) \{}
                \hlstd{ordered_column} \hlkwb{=} \hlkwd{tail}\hlstd{(}\hlkwd{bubble_LOF}\hlstd{(distance_matrix[,column],}
                        \hlnum{1}\hlopt{:}\hlkwd{ncol}\hlstd{(distance_matrix)),} \hlopt{-}\hlnum{1}\hlstd{)}
...
\end{knitrout}
						...
						El siguiente paso es calcular la densidad de cada punto. Esta se realiza mediante la siguiente expresión\footnote{Se entenderá por cardinal del conjunto $A$ a $|A|$. }: 
