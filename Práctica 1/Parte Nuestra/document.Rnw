...
distanciasordenadas = bubble(distancias)
distanciasordenadas
...
mode = function(list){
	...
	for (i in 1:n){
		...
		if (actual_c > times_mode){
			times_mode = actual_c
			actual_mode = actual_element
		}
	}
	actual_mode
}
