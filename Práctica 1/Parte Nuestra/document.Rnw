...
confidence = function(table, comb) {
	for (i in 2:len(table[1,])) { #itera dimensiones k
		separar <- Filter(function(x) length(x)==i, comb)
		
		for (j in 1:len(separar)) { #itera elementos de dim k
		
			for (k in 1:len(separar[j])) { #itera posibilidades que haya en el lado izquierdo
				#len(separar[j]) == i ?????
			
				lado = lapply(k, function(m) {combn(separar[j], m=m, simplify=TRUE)})
				
				
				for (n in 1:len(lado[,1])) { #
				
					otro_lado = dif(separar[j], lado[,n])
					hola = c(lado[n], otro_lado)
					print(hola)
				}
			}
		}
