...
\hlstd{mode} \hlkwb{=} \hlkwa{function}\hlstd{(}\hlkwc{list}\hlstd{)\{}
        ...
        \hlstd{actual_mode} \hlkwb{=} \hlstd{ordered_list[}\hlnum{1}\hlstd{]}
        \hlstd{times_mode} \hlkwb{=} \hlnum{0}
        \hlkwa{for} \hlstd{(i} \hlkwa{in} \hlnum{1}\hlopt{:}\hlstd{n)\{}
                \hlstd{actual_element} \hlkwb{=} \hlstd{ordered_list[i]}
                \hlstd{actual_c} \hlkwb{=} \hlnum{0}
                \hlstd{j} \hlkwb{=} \hlstd{i}
                \hlkwa{while}\hlstd{(j} \hlopt{<=} \hlstd{n} \hlopt{&} \hlstd{actual_element} \hlopt{==} \hlstd{ordered_list[j])\{}
                        \hlstd{actual_c} \hlkwb{=} \hlstd{actual_c} \hlopt{+} \hlnum{1}
                        \hlstd{j} \hlkwb{=} \hlstd{j}\hlopt{+}\hlnum{1}
                \hlstd{\}}
                \hlkwa{if} \hlstd{(actual_c} \hlopt{>} \hlstd{times_mode)\{}
                        \hlstd{times_mode} \hlkwb{=} \hlstd{actual_c}
                        \hlstd{actual_mode} \hlkwb{=} \hlstd{actual_element}
                \hlstd{\}}
        \hlstd{\}}
        \hlstd{actual_mode}
\hlstd{\}}
