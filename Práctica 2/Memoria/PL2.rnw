		 ...
		 \section{Clasificación no supervisada}
		 	...
		 	\subsection{Clusterización jerárquica aglomerativa}
		 		...
		 		{\small
		 			...
		 			tree = fcd_ahc(sample, "MIN")
		 			plot(as.dendrogram(tree), center = TRUE, yaxt='n')
			 		@
			 		<<arbol1, fig.cap = 'Clusterización con \\texttt{MIN} como árbol'>>=
			 		plot(tree)
			 		@
		 		}
		 		{\small
		 			...
		 			(sample = read.xlsx("data/datosKMeans.xlsx"))
		 			tree2 = fcd_ahc(sample, "MAX")
		 			plot(as.dendrogram(tree2), center = TRUE, yaxt='n')
		 			@
		 			<<arbol2, fig.cap = 'Clusterización con \\texttt{MAX} como árbol'>>=
		 			plot(tree2)
		 			@
		 		}
		 		{\small
		 			...
		 			(sample = read.xlsx("data/datosKMeans.xlsx"))
		 			tree3 = fcd_ahc(sample, "AVG")
		 			plot(as.dendrogram(tree3), center = TRUE, yaxt='n')
		 			@
		 			<<arbol3, fig.cap = 'Clusterización con \\texttt{AVG} como árbol'>>=
		 			plot(tree3)
		 			@
		 		}
 		...
 		\section{Clasificación supervisada}
 			...
 			\subsection{Árboles de decisión}
	 			...
	 			get_gain = function(sample, left, right, criteria, col, measure) {
	 				...
	 				impurity_father = get_impurity(sample, col, union(left, right), 
	 					criteria, equivalence_clases, measure)
	 				impurity_left = get_impurity(sample, col, left, 
	 					criteria, equivalence_clases, measure)
	 				impurity_right = get_impurity(sample, col, right, 
	 					criteria, equivalence_clases, measure)
	 				impurity_father - (len(left) / n * impurity_left + 
	 					len(right) / n * impurity_right)
	 			}
	 			@
	 			}
	 				...
	 				while (nrow(sample) > 0 && best_gain != 0) {
	 					...
	 					for (col in classes) {
	 						for (element in get_elements(sample, criteria)) {
	 							clasification = create_classification(sample, col, 
	 								element, criteria, measure)
	 					...
	 					if (best_gain != 0) {
	 						sample = subset(sample, sample[,best_clasification$parent] 
	 							%in% unlist(best_clasification$left))
	 						final_clasification = rbind(final_clasification, 
	 							best_clasification)
	 						if (details) {
	 							cat("\nIteración", nrow(final_clasification))
	 							cat("\n============\n")
	 							cat("Se escoge el nodo",best_clasification$parent,
	 								 "con una ganancia de", best_gain)
	 							cat("\nLos sucesos que tengan el/los valor/es",
	 								 paste(best_clasification$right),
	 							 "en ese nodo se clasifican como", 
	 							 	best_clasification$right_element)
	 							cat("\nLa nueva muestra es:\n")
	 							print(sample)
	 							cat("\n")
	 						}
	 			...
	 			<<fig.cap="Árbol de clasificación">>=
	 			plot(tree)
	 			@
 			...
 			\subsection{Regresión lineal}
 				...
 				fcd_regression = function(sample, d) {
 					...
 					colors[outliers] = "red"
 					plot(X, Y, col=colors, pch=21, bg=colors, 
 						main="Recta de regresión", xlab="Eje X", ylab="Eje Y")
 				...
 				<<recta4, fig.cap = "Recta con pendiente positiva">>=
 				sample4 = read.xlsx("../Memoria/data/conj4.xlsx", colNames=FALSE)
 				fcd_regression(sample4, 1.5)
 				@
